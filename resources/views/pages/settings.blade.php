                        ...
                        <div class="lg:w-2/4 p-2 w-full p-6  ">
                            ...
                            <form action="{{route('rrss.update')}}" method="post">
                                ...
                                <div class="grid  p-2">
                                    ...
                                    <input type="text" name="{{$item->name}}" class="rounded focus:ring-green-300"
                                        @if($item->url != 'none' ) value="{{$item->url}}" @endif>
                                    <label for="" class="flex items-center p-2 text-white">
                                        <input type="checkbox" name="{{$item->name}}_c" @if($item->active) checked
                                        @endif
                                        class="mr-2">Active</label>
                                </div>
                                @endforeach
                                <button type="submit" class="bg-indigo-500  px-4 p-2 w-full text-white">{{__('Save')}}</button>
                            </form>
